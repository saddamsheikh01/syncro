...
const SocialIconButton = ({ icon, label, color, onClick }: SocialButtonProps) => (
  <button
    type="button"
    onClick={onClick}
    className="group flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-muted transition-opacity hover:opacity-80"
    aria-label={label}
  >
    <span className="block" style={{ color: color as string }}>
      {icon}
    </span>
  </button>
);
