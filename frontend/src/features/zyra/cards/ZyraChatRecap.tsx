...
export const ZyraChatRecap = ({
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      setError(t("Unable to delete chats. Try again."));
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    ...
    >
          ...
          {loading ? (
            <div className="flex items-center gap-2 py-4 text-subtle">
              ...
              <span className="text-sm">
                {t("Zyra is analyzing your chats...")}
              </span>
            </div>
          ) : error ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {data.recentContacts.slice(0, 5).map((name, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-2 rounded-full border border-border/70 bg-surface px-3 py-1.5"
                      >
        ...
        {!loading && !error && data ? (
          <div className="flex items-center justify-between border-t border-border/40 pt-3">
            <p className="text-[10px] text-subtle">
              {t("Generated by Zyra based on your conversations")}
            </p>
            <div className="flex items-center gap-2">
              <Button
                size="sm"
      ...
      <Modal
        open={confirmOpen}
        title={t("Delete all Zyra chats?")}
        description={t("This action is final. All Zyra sessions will be deleted.")}
