...
export const ZyraProfileRecap = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError("Error while saving. Try again.");
    } finally {
      setSaving(false);
    }
  };
          ...
          {loading ? (
            <div className="flex items-center gap-2 py-2 text-subtle">
              ...
              <span className="text-sm">
                {userId
                  ? "Zyra is analyzing this profile..."
                  : "Zyra is analyzing your profile..."}
              </span>
            </div>
          ) : error && !displayRecap ? (
        ...
        {!loading && !isEditing && displayRecap ? (
          <div className="flex items-center justify-between border-t border-border/40 pt-3">
            <p className="text-[10px] text-subtle">
              {userId
                ? "Generated by Zyra based on the profile"
                : "Generated by Zyra based on your profile"}
            </p>
            <div className="flex gap-2">
              {isOwnProfile && (
                <Button
