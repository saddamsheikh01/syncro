...
import { getTestRecap } from "@/services/zyra";

export interface ZyraTestRecapProps
...
export const ZyraTestRecap = ({
  className,
  submissionId,
  title = "Ecco cosa ho capito di te",
  ...props
}: ZyraTestRecapProps) => {
          ...
          {loading ? (
            <div className="flex items-center gap-2 py-2 text-subtle">
              ...
              <span className="text-sm">
                Zyra sta analizzando le tue risposte...
              </span>
            </div>
          ) : error ? (
        ...
        {!loading && !error && recap ? (
          <div className="flex items-center justify-between border-t border-border/40 pt-3">
            <p className="text-[10px] text-subtle">
              {testTitle
                ? `Generato da Zyra in base a "${testTitle}"`
                : "Generato da Zyra in base alle tue risposte"}
            </p>
            <Button
              size="sm"
              variant="ghost"
